...
const DB_PATH =
  process.env.DB_PATH ||
  path.join(process.cwd(), "..", "data", "election_odds.db");
...
export function getChartData(
  ...
  for (const row of rows) {
    ...
    const name = extractChartCandidateName(
 ...
 */
function extractChartCandidateName(
  contractName: string,
  source: string,
): string | null {
...
function extractCandidateName(
  ...
  if (willMatch) {
    ...
    // Check for party names (handles Kalshi typo "Democratics")
...
function getCanonicalMarketType(marketName: string): string | null {
  ...
  if (
    (lower.includes("presidential election winner") ||
      ...
      lower.includes("us presidency 2028")) &&
...
function getChartBasedPriceChanges(
  ...
  const currentPrices = db.prepare(currentPricesQuery).all(dbMarketId) as {
    contract_name: string;
    yes_price: number;
  }[];
  const historicalPrices = db
    .prepare(historicalPricesQuery)
    .all(changeTimestamp, dbMarketId) as {
    contract_name: string;
    yes_price: number;
  }[];
...
export function getMarkets(options?: {
  ...
  for (const [canonicalType, relatedMarkets] of marketsByType) {
    ...
    const eboMarket = relatedMarkets.find(
      (m) => m.source === "electionbettingodds",
    );
    ...
    for (const dbMarket of relatedMarkets) {
      const contractsWithPrices = db
        .prepare(contractsWithPricesQuery)
        .all(dbMarket.market_id, dbMarket.source) as (DbContract & {
        yes_price: number;
        no_price: number | null;
        volume: number | null;
        snapshot_time: string;
      ...
      for (const contract of contractsWithPrices) {
        ...
        const isPartyMarket =
          canonicalType.includes("party") || canonicalType.includes("control");
        if (!isPartyMarket && isPartyName(candidateName)) {
          continue;
        }
        // For party markets, normalize the party name
        const displayName = isPartyMarket
          ? normalizePartyName(candidateName)
          : candidateName;
...
function excludeIlliquidSources(
  ...
  for (const source of allSources) {
    ...
    // If all contracts from this source are within 5% of each other, it's illiquid
    if (spread < 0.05) {
      // Only remove if other sources exist for these contracts
...
/**
 * Get state senate race markets (SQLite version)
 */
export function getStateSenateRaces(options?: {
  states?: string[];
  changePeriod?: string;
}): Market[] {
  const db = getDb();
  ...
  for (const abbrev of stateAbbrevs) {
    ...
    const marketsResult = db
      ...
      FROM markets m
      WHERE LOWER(m.market_name) LIKE ?
        AND LOWER(m.market_name) LIKE '%senate%'
        AND LOWER(m.market_name) NOT LIKE '%primary%'
        AND LOWER(m.market_name) NOT LIKE '%nomin%'
        AND LOWER(m.market_name) NOT LIKE '%matchup%'
        AND LOWER(m.market_name) NOT LIKE '%endorse%'
        AND LOWER(m.market_name) NOT LIKE '%closer%'
        AND LOWER(m.market_name) NOT LIKE '%district%'
        AND LOWER(m.market_name) NOT LIKE '%state senate%'
        AND m.source IN ('Polymarket', 'Kalshi', 'PredictIt', 'Smarkets')
      ORDER BY m.total_volume DESC NULLS LAST
    `,
      )
      .all(`%${stateNameLower}%`) as DbMarket[];

    if (marketsResult.length === 0) continue;

    const contractsByCandidate = new Map<string, Contract>();

    // Build historical prices map
    const historicalByCandidate = new Map<string, number[]>();

    for (const dbMarket of marketsResult) {
      // Get current prices
      const contractsWithPrices = db
        .prepare(
          `
      ...
      const historicalPrices = db
        .prepare(
          `
        SELECT
          c.contract_id, c.contract_name, ps.yes_price
        FROM contracts c
        INNER JOIN (
          SELECT contract_id, market_id, source, yes_price, snapshot_time,
                 ROW_NUMBER() OVER (PARTITION BY source, market_id, contract_id ORDER BY snapshot_time DESC) as rn
      ...
      `,
        ...
        .all(changeTimestamp, dbMarket.market_id, dbMarket.source) as {
        contract_id: string;
        contract_name: string;
        yes_price: number;
      }[];
      ...
      for (const row of historicalPrices) {
        ...
        historicalByCandidate.get(normalizedName)!.push(row.yes_price);
      }
      ...
      for (const contract of contractsWithPrices) {
        ...
        const displayName = normalizePartyName(candidateName);
        const normalizedName = normalizeCandidateName(displayName);
    ...
    // If we have both party-level contracts and individual candidates, keep party entries for consistency
    const PARTY_KEYS = [
      "republican party",
      "democratic party",
      "independent",
...
/**
 * Get state governor race markets (SQLite version)
 */
export function getStateGovernorRaces(options?: {
  states?: string[];
  changePeriod?: string;
}): Market[] {
  const db = getDb();
  ...
  for (const abbrev of stateAbbrevs) {
    ...
    // Find markets that mention this state + governor, excluding primaries/nominations
    const marketsResult = db
      .prepare(
        `
      SELECT DISTINCT
      ...
      FROM markets m
      WHERE LOWER(m.market_name) LIKE ?
        AND (LOWER(m.market_name) LIKE '%governor%'
    ...
    // If we have both party-level contracts and individual candidates, keep party entries
    const PARTY_KEYS = [
      "republican party",
      "democratic party",
      "independent",
...
/**
 * Get senate primary markets (SQLite version)
 */
export function getSenatePrimaries(options?: {
  changePeriod?: string;
}): Market[] {
  const db = getDb();
  const changePeriod = options?.changePeriod || "1d";
  ...
  // Find all senate primary/nomination markets
  const marketsResult = db
    .prepare(
      `
    SELECT DISTINCT
  ...
  if (marketsResult.length === 0) return [];

  // Group markets by state + party
  const primaryGroups = new Map<
    string,
    { state: string; stateAbbrev: string; party: string; dbMarkets: DbMarket[] }
  >();
  ...
  for (const [groupKey, group] of primaryGroups) {
    ...
    const category = (
      partyLower === "democratic" ? "senate-primary-dem" : "senate-primary-gop"
    ) as MarketCategory;
...
export function getStats() {
  ...
  const marketCount = db
    .prepare("SELECT COUNT(DISTINCT market_id) as count FROM markets")
    .get() as { count: number };
  ...
  try {
    ...
    try {
      content = readFileSync(
        path.join(process.cwd(), "data", "curated-posts.csv"),
        "utf-8",
      );
    } catch {
