...
export function getStateSenateRaces(options?: {
  ...
  for (const abbrev of stateAbbrevs) {
    ...
    const marketsResult = db.prepare(`
      SELECT DISTINCT
    ...
    for (const dbMarket of marketsResult) {
      // Get current prices
      const contractsWithPrices = db.prepare(`
        SELECT
      ...
      `).all(dbMarket.market_id, dbMarket.source) as (DbContract & { yes_price: number; no_price: number | null; volume: number | null; snapshot_time: string })[];
      ...
      const historicalPrices = db.prepare(`
        ...
        WHERE c.market_id = ? AND c.source = ?
        AND ps.yes_price IS NOT NULL
      `).all(changeTimestamp, dbMarket.market_id, dbMarket.source) as { contract_id: string; contract_name: string; yes_price: number }[];
...
export function getSenatePrimaries(options?: {
  ...
  const primaryGroups = new Map<string, { state: string; stateAbbrev: string; party: string; dbMarkets: DbMarket[] }>();
