...
function RaceCard({ race }: { race: Market }) {
  ...
  return (
    <Link href={`/senate/${stateAbbrev}`} className="block">
...
export default function SenateRacesPage() {
  ...
  return (
    <div className="container py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">2026 Senate Races</h1>
        <p className="text-muted-foreground">
          All {races?.length || 0} Senate races up in 2026, plus primaries.
          Aggregated from Polymarket, Kalshi, and PredictIt.
        </p>
      </div>
        ...
        <section className="mb-10">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {primaries.map((primary) => (
              <PrimaryCard key={primary.id} primary={primary} />
            ))}
          </div>
        </section>
      )}
